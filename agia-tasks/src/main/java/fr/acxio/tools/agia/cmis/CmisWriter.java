...
public class CmisWriter implements ItemStreamWriter<NodeList> {
        ...
        if (!sItems.isEmpty()) {
            ...
            for (NodeList aNodeList : sItems) {
                for (Node aNode : aNodeList) {
                    aCurrentNodePath = ISO9075.decode(aNode.getPath().replaceAll("[^:/]*:", "")); // Strip
                                                                                                  // namespaces
    ...
    private org.apache.chemistry.opencmis.client.api.Folder getParent(String sNodePath) {
        ...
        org.apache.chemistry.opencmis.client.api.Folder aParent = (org.apache.chemistry.opencmis.client.api.Folder) cmisService.getSession().getObjectByPath(
                aParentPath, operationContext);
        if (aParent == null) {
