...
/**
 * <p>Item reader for hibernate stored nodes.</p>
 * <p>Item readers for hibernate use to lack efficiency because the whole objects
 * trees is stored into the step context.</br>
 * The HibernateNodeReader will read nodes' ids only, and a dedicated processor
 * will retrieve the nodes themselves.</p>
 * <p>The {@code currentStep} property represents the node lifecycle steps.</br>
 * It can take any integer value, and is used to select the nodes at the
 * given step.</br>
 * By convention, positives values represent successful steps and negatives
 * ones represent failed steps.</br>
 * Some default values are provided by
 * {@link fr.acxio.tools.agia.alfresco.domain.NodeStatus NodeStatus}.</p>
 * <p>The {@code sqlQuery} property can be altered with extreme care to get
 * a specific collection of nodes.
 * </p>
 * 
 * @author pcollardez
 *
...
public class HibernateNodeReader implements ItemReader<ProcessIndicatorItemWrapper<Node>>, StepExecutionListener, InitializingBean, DisposableBean {
	 ...
	 * @param sSqlQuery the new SQL query.
	 */
	...
	private List<Long> retrieveKeys() {
		...
		synchronized (lock) {
			...
			return jdbcTemplate.query(sqlQuery, aParams,
